# Security Keywords Summary

- **encode**: Convert data into another format for compatibility or transmission (for example, Base64). Encoding is **not** a security control by itself.
- **decode**: Reverse an encoding process to recover the original representation from an encoded format.
- **compress**: Reduce data size to save storage or bandwidth by removing redundancy (for example, ZIP, gzip).
- **decompress**: Restore compressed data back to its original form.
